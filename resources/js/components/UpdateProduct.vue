...
export default {
    ...
    methods: {
            ...
            axios.post('/product', product).then(response => {
                ...
                if (error.response.data.errors) {
                    Object.keys(error.response.data.errors).map(item => {
                        if (error.response.data.errors[item][0]) this.$toasted.error(error.response.data.errors[item][0]);
                    })
                } else {
        ...
        showSuccess(file, response) {
            if (!response.error) this.$toasted.error(response.message);
            this.images.push({"file": response.filename, "folder": response.folder});
        }
    },
    mounted() {
